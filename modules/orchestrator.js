import {
  ...
  searchStats,
  getFullDataJson,
} from "./RESTManagement.js";
import {
  updateTauxGraph,
  updateTauxGraphModal,
} from "./tauxSelectiviteGraph.js";
import {
  updateComparaisonSexe,
  updateComparaisonSexeModal,
} from "./comparaisonSexe.js";
import {
  updateProcessusSelection,
  updateProcessusSelectionModal,
} from "./processusSelection.js";
import {
  updatePropositionDiplomeOrigine,
  updatePropositionDiplomeOrigineModal,
} from "./propositionDiplomeOrigineGraph.js";
import {
  updateRepartitionDiplomeOrigine,
  updateRepartitionDiplomeOrigineModal,
} from "./repartitionDiplomeOrigineGraph.js";
...
export async function afficherDetailsFormation(ifc) {
  try {
    ...
    if (baliseAlternance) {
      baliseAlternance.textContent =
        Number(data_formation.alternance) === 1 ? "Alternance" : "Initial";
    }
    ...
    if (baliseDesc) {
      if (localData && localData.desc) {
        ...
        baliseDesc.textContent =
          "Description indisponible pour cette formation.";
      }
    }
    ...
    if (!isNaN(lat) && !isNaN(lon)) {
      ...
      if (mapDiv)
        mapDiv.innerHTML =
          '<p style="padding:20px; text-align:center;">Carte indisponible.</p>';
    }
    ...
    if (BaliseAdresse) {
      if (localData && localData.adresse) {
        const adresseEncoded = encodeURIComponent(localData.adresse);
        BaliseAdresse.innerHTML = `<a href="https://www.google.com/maps/search/?api=1&query=${adresseEncoded}" target="_blank">${localData.adresse}</a>`;
      } else {
        BaliseAdresse.textContent =
          "Adresse indisponible pour cette formation.";
      }
    }
    ...
    if (BaliseMail) {
      if (localData && localData.mail) {
        ...
        BaliseMail.href = `https://mail.google.com/mail/?view=cm&to=${localData.mail}`;
        BaliseMail.target = "_blank"; // ouvre dans un nouvel onglet
      } else {
        BaliseMail.textContent = "Mail indisponible pour cette formation.";
        BaliseMail.removeAttribute("href");
      }
    }
    ...
    if (localData && localData.tag) {
      ...
      document.querySelector(".right-item__formation-container").innerHTML =
    ...
    if (uai && ifc) {
      ...
      if (
        statsData.candidatures[0]["general"] &&
        statsData.candidatures[0]["experience"]
      ) {
        const L3 = statsData.candidatures[0]["experience"]["lg3"]["prop"];
        const LP3 = statsData.candidatures[0]["experience"]["lp3"]["prop"];
        const master =
          statsData.candidatures[0]["experience"]["master"]["prop"];
        const ninscrit =
          statsData.candidatures[0]["experience"]["noninscrit"]["prop"];
        ...
        console.warn(
          "Aucune statistique pour candidatures[général] ou candidatures[experience]"
        );
      ...
      if (
        statsData.candidatures[0]["general"] &&
        statsData.candidatures[0]["origine"]
      ) {
        const L3 = statsData.candidatures[0]["experience"]["lg3"]["accept"];
        const LP3 = statsData.candidatures[0]["experience"]["lp3"]["accept"];
        const master =
          statsData.candidatures[0]["experience"]["master"]["accept"];
        const ninscrit =
          statsData.candidatures[0]["experience"]["noninscrit"]["accept"];
        const autre =
          statsData.candidatures[0]["experience"]["autre"]["accept"];
        ...
        console.warn(
          "Aucune statistique pour candidatures[général] ou candidatures[origine]"
        );
...
async function updateFormationsSimilaires(currentTag, currentIfc) {
  ...
  try {
    ...
    const similarCandidates = allData.filter(
      (item) => item.tag === currentTag && item.ifc !== currentIfc
    );
